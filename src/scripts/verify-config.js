...
module.exports.run = async (config) => {
	...
	// Make sure Node.js V14 or higher is being ran.
		...
		await client.login(config.token).catch((e) => {
			if (e.message == 'An invalid token was provided.') {
				...
				logger.error(
					`${chalk.red(
						'✗'
					)} You need to enable privileged intents on the discord developer page.`
				)
				return true
			}
		})
	...
	if (!config.MongoDBURl) {
		...
		await mongoose
			.connect(config.MongoDBURl, { useUnifiedTopology: true, useNewUrlParser: true })
			.catch((err) => {
				console.log(err)
			...
			'grant_type=client_credentials',
			{
				headers: {
					Authorization: `Basic ${Buffer.from(
						`${config.api_keys.spotify.iD}:${config.api_keys.spotify.secret}`
					).toString('base64')}`,
